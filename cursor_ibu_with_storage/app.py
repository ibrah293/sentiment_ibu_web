...
EMOTION_MODEL_PATH = 'best_emotion_model.pkl'
EMOTION_VECTORIZER_PATH = 'emotion_vectorizer.pkl'

def load_or_create_models():
    """
    Load existing models or create new ones if they don't exist
    """
    global sentiment_models, emotion_models, preprocessor, vectorizer, emotion_vectorizer
    
    # Initialize preprocessor
    preprocessor = TextPreprocessor()
    
    # Try to load trained sentiment model and vectorizer
    if os.path.exists(SENTIMENT_MODEL_PATH) and os.path.exists(VECTORIZER_PATH):
        print('Loading trained sentiment model and vectorizer...')
        best_model = SentimentClassifier.load_model(SENTIMENT_MODEL_PATH)
        vectorizer = TextVectorizer.load_vectorizer(VECTORIZER_PATH)
        # Only one best model, use key 'best'
        sentiment_models.clear()
        sentiment_models['best'] = best_model
        # Alias all model types to the best model
        sentiment_models['logistic_regression'] = best_model
        sentiment_models['random_forest'] = best_model
        sentiment_models['naive_bayes'] = best_model
        print('Loaded best sentiment model and vectorizer.')
        # Optionally: Load emotion models from disk if you have them saved
        # Always train emotion models on sample data for demo
        ...
        X_emotion = vectorizer.transform(processed_df['processed_text'])
        ...
        for model_type in emotion_model_types:
            ...
            model.fit(X_emotion, y_emotion)
            emotion_models[model_type] = model
    else:
        print('Trained model/vectorizer not found. Training on sample data...')
        ...
        # Train emotion models (only when X is defined)
        emotion_model_types = ['logistic_regression', 'random_forest', 'naive_bayes']
        for model_type in emotion_model_types:
            model = EmotionClassifier(model_type)
    ...
    if os.path.exists(EMOTION_MODEL_PATH) and os.path.exists(EMOTION_VECTORIZER_PATH):
        print('Loading trained emotion model and vectorizer...')
        best_emotion_model = EmotionClassifier.load_model(EMOTION_MODEL_PATH)
        emotion_vectorizer = TextVectorizer.load_vectorizer(EMOTION_VECTORIZER_PATH)
        emotion_models.clear()
        emotion_models['best'] = best_emotion_model
        emotion_models['logistic_regression'] = best_emotion_model
        emotion_models['random_forest'] = best_emotion_model
        emotion_models['naive_bayes'] = best_emotion_model
        print('Loaded best emotion model and vectorizer.')
    else:
        # If not available, already trained on sample data above (or will be trained in else block)
        # For compatibility, use the main vectorizer for emotion as well
...
def analyze_sentiment():
    ...
    try:
        ...
        text = data.get('text', '')
        model_type = data.get('model_type', 'best' if 'best' in sentiment_models else 'logistic_regression')
...
def analyze_emotion():
    ...
    try:
        ...
        # Vectorize text (use emotion_vectorizer if available)
        X = emotion_vectorizer.transform([processed_text])
